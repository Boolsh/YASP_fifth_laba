...
public:
	...
	Employee() {};
	Employee(std::string _name, std::string _surname, std::string _patronymic,
		std::chrono::year_month_day _applyment_date,
		std::optional <std::chrono::year_month_day> _fire_date,
		Posts _post, Sex _sex, int _salary)
	{
		name = _name;
		surname = _surname;
		patronymic = _patronymic;
		applyment_date = _applyment_date;
		if (_fire_date.has_value())
			fire_date = _fire_date;
		post = _post;
		sex = _sex;
		salary = _salary;
	}
	...
	friend std::istream& operator>>(std::istream& in, Employee& emp) {
		...
		// Чтение имени
		while (true) {
			//if (in == std::cin) std::cout << "Введите имя: ";
		...
		// Чтение фамилии
		while (true) {
			//if (in == std::cin) std::cout << "Введите фамилию: ";
		...
		// Чтение отчества (может быть пустым)
		//if (in == std::cin) std::cout << "Введите отчество: ";
		std::getline(in >> std::ws, tmp.patronymic);

		// Чтение даты приема
		while (true) {
			//if (in == std::cin) std::cout << "Введите дату приема (ГГГГ-ММ-ДД): ";
			std::string date_str;
		...
		while (true) {
			//if (in == std::cin) std::cout << "Введите дату увольнения (ГГГГ-ММ-ДД или - если нет): ";
		...
		// Чтение должности
		while (true) {
			//if (in == std::cin) std::cout << "Введите должность (Cleaner, Cook, Manager, Developer, HR, Analyst): ";
		...
		// Чтение пола
		while (true) {
			//if (in == std::cin) std::cout << "Введите пол (Male, Female, Frog): ";
		...
		// Чтение зарплаты
		while (true) {
			//if (in == std::cin) std::cout << "Введите зарплату: ";
			if (!(in >> tmp.salary)) {
				if (in.eof()) return in;
				clear_istream(in);
